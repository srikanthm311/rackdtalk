...
class Main extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data['mywins']           = $this->Common_model->query_array('SELECT qcbw.*,qcv.comment,qcv.question FROM `rt_tb_comment_bid_winners` qcbw inner join rt_vw_comment qcv on qcbw.comment_id=qcv.id WHERE qcbw.`user_id` =2  order by  qcbw.id desc');
		$this->data['notifications']    = $this->getNotifications($this->userID);
		}
		...
		$this->data['bestAnsweredWinners']          = $this->Users_model->getBestAnsweredWinners();
	...
	{
			return $this->Common_model->query_array('select * from rt_tb_notifications where isview=0 AND (user_id=0 OR user_id='.$this->userID.')');
	}
    ...
    public function forgot_pwd()
	{
		    ...
		    {
		    $data['user_name'] = $this->input->post('forgot_pwd');
			$user=$this->Common_model->get_table_row('tr_tb_user',array('email_id' =>  $data['user_name'],'is_active'=>1 ,'is_delete'=>0));
			if($user)
			{
					            ...
					            $unique 		=	$this->common->random_generator(6);
								$this->Common_model->update_table('tr_tb_user',array('password' => md5($unique)),array('id' =>  $user['id']));
								$email_data = array('<#USERNAME#>'		=>	$user['email_id'],
													'<#PASSWORD#>'		=>	$unique ,
													'<#BASEURL#>'			=>	base_url(),
												);
								$message  = $this->common->getEmailText($email_data,'forgot-password.txt');
								...
								$this->email->message($message[1]);
								$this->email->send();
				echo json_encode(array('success' => 'success','color' => 'json-success','msg' => 'Accont Details sent to your mail inbox/mobile number'));exit;
				
				//  $this->session->set_flashdata('msg', 'Accont Details sent to your mail inbox/mobile number');

			}
			else
			{  //$this->session->set_flashdata('msg', 'Email/Mobile No not exists in our Database');
			   echo json_encode(array('success' => 'success','color' => 'json-error','msg' => 'Email/Mobile No not exists in our Database'));exit;
			}
		}
	}
	...
	public function resetpassword()
	{
	     ...
	     if($this->input->post())
		 {
		   ...
		   $this->session->set_flashdata('message', '<b style="color:green">Password changed successfully</b>');
		   }else{
			$this->session->set_flashdata('message', '<b style="color:red">Your current password is invalid</b>');
		   }
	...
	{   
		...
		$this->data['pageHeading'] = $this->lang->line('about_to_close_questions');
	...
	{
		$this->load->model('Question_model');
		$this->data['pageHeading'] = $this->lang->line('trending_questions');
	...
	{
		$this->load->model('Question_model');
		$this->data['pageHeading'] = $this->lang->line('latest_questions');
	...
	{
		$this->load->model('Question_model');
		$this->data['pageHeading'] = $this->lang->line('top_bidded_questions');
	...
	{
		$this->load->model('Question_model');
		$this->data['pageHeading'] = $this->lang->line('category_questions');
	...
	public function my_withdraw()
	{
		$this->isLoggedIn();
		$this->load->model('Question_model');
		$this->data['pageHeading'] = 'Money Withdraw';
		$this->data['pageKey'] = 'mow';
		$cnd_arr['user_id']=$this->userID;
		if($this->input->post())
		{
		$post= $this->input->post();
		$withdrawListcount= $this->Users_model->getAllWithdrawalscount($cnd_arr);
		if($withdrawListcount < 3 ){
		if($post['wallet_type']=='talker'){
		$wallet_amt = $this->data['wallet']['talker_credit_amount']- $this->data['wallet']['talker_debit_amount'];
		}
		elseif($post['wallet_type']=='rocker')
		{
			$wallet_amt = $this->data['wallet']['rocker_credit_amount']- $this->data['wallet']['rocker_debit_amount'];
			}
		if($wallet_amt >= $post['w_amount'])
		{
			$data = array('user_id'=>$cnd_arr['user_id'],'amount'=>$post['w_amount'],'type'=>$post['wallet_type']);
			$lastwithdraw=$this->Common_model->insert_table('rt_tb_withdraw',$data);
			$this->session->set_flashdata('message', '<span style="color:red">your request sent for Admin Approval</span>');
		}
		else{
				$this->session->set_flashdata('message', '<span style="color:red">Insufficient Balance</span>');
			}
		}
		else{
			$this->session->set_flashdata('message', '<span style="color:red">your reached Maximum no of withdrawals</span>');
			}
		}
		$this->data['withdrawList']= $this->Users_model->getAllWithdrawals($cnd_arr);
		//echo '<pre>';print_r($this->data['withdrawList']);exit;
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/my-withdraw',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	}
	...
	public function mywins()
	{
		$this->isLoggedIn();
		$this->load->model('Question_model');
		$this->data['pageHeading'] = 'My Winning';
		$this->data['pageKey'] = 'myw';
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/mywins',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	}
	...
	public function addBidonComment()
	{
		...
		{ 
			...
			if($amount>=$nextHighestBid)
			{
			    ...
			    $this->leadbidnotification($commentID);
	...
	public function comment_page($qid,$cid='')
	{
		...
		$this->data['stitle']=($this->data['commentInfo']['data'][$qid]['type']==1)?$this->data['commentInfo']['data'][$qid]['question']:'';
		$this->data['simage']=($this->data['commentInfo']['data'][$qid]['type']==2)?base_url().$this->data['commentInfo']['data'][$qid]['question']:'';
		$this->data['sdesc']=$this->data['commentInfo']['comments'][$qid][$cid]['comment'];
		//echo '<pre>';print_r($this->data['commentInfo']);exit;
		$this->load->view('frontend/includes/header',$this->data);
	...
	public function post_your_question()
	{
		...
		if($this->input->post())
		{
			...
			$this->Question_model->updateQuestion($post);unset($_SESSION['selectSegment']);
			$this->session->set_flashdata('message', '<span style="color:green">Question Submited Successfully.</span>');
			redirect(base_url('post-your-question.html'));exit;
		}
		if(!isset($_SESSION['selectSegment']))
		{
			 redirect(base_url('main/segment')); exit;
	...
	{
            $questionsList['suggestions']=array();
			if($this->isAjaxRequest())
			{   
                $post=$this->input->post(); $query=$this->input->get('query');
			    $questions =$this->Common_model->query("SELECT question FROM rt_tb_question WHERE is_active=1 AND is_delete=0 AND  question LIKE '".$query."%' LIMIT 10");
				foreach($questions as $question)
			    {
				$questionsList['suggestions'][]= array('value' =>$question->question,'data' => $question->question);
			    }
			   header('Content-Type: application/json');
			   echo json_encode($questionsList);exit;	
			}
	 ...
	 public function segment($id='')
	 {
		...
		if($id!='')
		{
			 $_SESSION['selectSegment'] = $id;
			 redirect(base_url('post-your-question.html')); exit;
		}
		$this->data['categoryList']=$this->Common_model->get_table('rt_tb_category',array('is_active' => 1,'is_delete' => 0),array('category_name','id','image'));
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/segment',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	 }
	...
	{
		$this->load->model('Question_model');
		$this->data['pageHeading'] = 'Winners';
		$this->data['pageKey'] = 'winners';
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/winners',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	}
	...
	public function plans()
	{
		...
		if($this->input->post())
		{
					    ...
					    $payment['order_id'] = "RDTTTAS1_" . $planid ;
	 ...
	 public function addFunds($ID)
	{
			...
			if($this->input->post())
		    {
					    ...
					    $payment['order_id'] = "RDTTTAS1_" . $tid ;
	...
	public function walletsuccess()
   {
    ...
    {
		$this->session->set_flashdata('message', '<span style="color:red">Payment Cancelled</span>');
		redirect(base_url()); exit;
	 }
	...
	public function failure()
   {
		if($this->input->post())
		{
			 ...
			 { 
				     $payment_id     = $post['mihpayid'];
			   	     $idDetails      = explode('_',$post['txnid']);
					 $this->Common_model->update_table('rt_tb_transactions',array('txn_uid'=>$payment_id,'status'=>'FAILED'),array('id' =>$idDetails[1])); 
					 $this->session->set_flashdata('message', '<span style="color:red">Payment Failed</span>');
					 redirect(base_url()); exit;
			 }
		}
	}
	 ...
	 public function addReportOnComment()
	{

			if($this->isAjaxRequest())
			{   
				$post=$this->input->post();
				$this->Common_model->update_table('rt_tb_question_comment',array('is_flag' => 1),array('id' =>$post['q'] ));
				echo json_encode(array('success' => 'success','message' => 'Your Report is Posted Successfully','html'=> $html));exit; 
			}
				  ...
				  $html='<div class="inner cover price comment_ques_bi"> <div class="img_var_come"><img src="'.$pic.'" width="30" height="30"> </div> <div class="comment_area_re"> <p><strong>You</strong></p> <p>'.$post['text'].'</p></div>
                     <div class="col-md-4">
                        <ul class="share share_one">
                            <li><i class="fa fa-share-alt" aria-hidden="true"></i> '.$this->lang->line('share').'</li>
                     ...
                     <div class="col-md-4">
                     <p class="bid_likes_comment"><a href="javascript:" class="newcomm" id="newcom-'.$cid.'" rel="'.$cid.'" rel1="likes" rel2="'.$post['q'].'">0 '.$this->lang->line('likes').'</a> </p>
                  </div></div>';
	 ...
	 public function insertLike()
	{
			...
			{   
				...
				$this->Common_model->insert_table('rt_tb_comments_likes_dislikes',array('comment_id'=>$_POST['q'],'user_id'=>$this->userID,'ip_address' =>$ip,'type' =>$_POST['type']));		    $this->leaddiscussionnotification($_POST['question'],$_POST['q']);
				echo 1;
				exit;  
			}
	...
	public function success()
   {
		if($this->input->post())
		{
			 ...
			 { 
					 ...
					 $this->session->set_flashdata('message', '<span style="color:green">Payment Successful.Now you are eligible for commenting.</span>');
					 redirect(base_url()); exit;
			 }
			 else redirect(base_url('main/cancel')); exit;
		}
	...
	public function bid_policy()
	 {
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/bid_policy',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	 }
	 ...
	 {
		$this->load->view('frontend/includes/header',$this->data);
		$this->load->view('frontend/refund_policy',$this->data);
		$this->load->view('frontend/includes/footer',$this->data);
	 }
	 	...
	 	public function profilePic()
	{
	 ...
	 public function leadbidnotification($commentID)
	{
		  if($commentID!='')
		  {
		   $commentInfo=$this->Common_model->query_array('Select qcb.user_id,qc.comment,amount from rt_tb_comment_bid as qcb inner join rt_tb_question_comment as qc on qcb.comment_id=qc.id where qcb.comment_id = '.$commentID.' order by amount desc limit 1,1');
		  $message = 'Some body leading your bid on comment "'.$commentInfo[0]['comment'].'" ';
		  if(!empty($commentInfo))
		  {
		  if($commentInfo[0]['user_id']!=$this->userID)
		  {
		  $this->Common_model->insert_table('rt_tb_notifications',array('message'=>$message,'user_id'=>$commentInfo[0]['user_id']));
		  }
		  }
		  }else  redirect(base_url(''));
	}
	
	public function leaddiscussionnotification($question,$commentID)
	{
		  if($commentID!='')
		  {
			  
		   $commentInfo=$this->Common_model->query_array('SELECT qc.id as comment_id,qc.comment as comment, qc.`user_id`,MAX(qvcl.comment_likes_count) as comment_likes_count FROM `rt_tb_question_comment` as qc inner join rt_vw_comment_likes_count as qvcl on qvcl.comment_id=qc.id WHERE qc.`qusetion_id` = '.$question.' order by MAX(qvcl.comment_likes_count) desc limit 0,1');
		   if(!empty($commentInfo))
		   {
		    $message = 'Your comment "'.$commentInfo[0]['comment'].'" is leading  as of now.';
		    if($commentInfo[0]['comment_id']==$commentID)
		    $this->Common_model->insert_table('rt_tb_notifications',array('message'=>$message,'user_id'=>$commentInfo[0]['user_id']));
		   }
		  /* else
		   {  
			  $commentInfo=$this->Common_model->get_table_row('rt_tb_question_comment',array('comment_id'=>$commentID),array('comment','user_id'));
			  $message = 'Your comment is leading  "'.$commentInfo['comment'].'" as of now.';  
			  $this->Common_model->insert_table('rt_tb_notifications',array('message'=>$message,'user_id'=>$commentInfo['user_id']));
		   }*/
		  }
	}
