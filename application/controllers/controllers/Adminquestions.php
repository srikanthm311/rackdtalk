...
class Adminquestions extends Admin {
	...
	public function post_your_question($question_id = '')
	{
		...
		if($this->input->post())
		{
			...
			if($post['qtype'] == 2){
				if($_FILES['question']['tmp_name']!=''){
					...
					$data = array('category_id' => $post['segment'],'question' => $post['question'],'description' => $post['description'],'tags' => $post['tags'],'type' => $post['qtype'],'comments_closed_at' => date('Y-m-d H:i:s'),'bidding_closed_at'=>date('Y-m-d H:i:s'),'is_trending'=>0,'updated_at' => date('Y-m-d H:i:s'),'is_active' =>1,'is_approved' =>1,'bidding_closed_at'=>$post['bidding_closed_at'],'comments_closed_at'=>$post['comment_closed_at']);
						}
				else{
			...
			$post['admin']=$this->session->userdata('adminID');
			if($_FILES['question']['tmp_name']!='')
			{
				$imageFileType = pathinfo($_FILES['question']['name'],PATHINFO_EXTENSION);
				if(($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"&& $imageFileType != "gif")|| $_FILES["image"]["size"] > 26214400) 
		...
		if(!empty($question_id)){
			...
			//$this->data['question_detials'] = array_shift($this->Common_model->get_table('rt_tb_question',array('id'=>$question_id,'is_delete'=> 0),'*','','',''));
		...
		//$post['comment_id'] = $this->data['CommentsList']['id'];
		//$this->data['bids'] = $this->Superadmin_model->getBids($post);
		//echo "<pre>"; print_r($this->data['bids']); exit;
