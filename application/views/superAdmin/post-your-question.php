                    ...
                    <div class="addmoney add_test">
                        ...
                        <input type="hidden" name="user_role" value="<?php echo $question_detials['role_id']?>" ID="user_role">
                          ...
                          <div class="form-group" style="float: left; width: 100%;">
                            ...
                            <label class="col-md-2 catogery_name">Closed</label>
                    
                            <div class="col-md-6">
                    
                              <input type="checkbox" class="form-contro" id="is_closed" name="is_closed" value="1" <?php echo ($question_detials['is_closed'])?'checked':''?>/>
                    
                            </div>
                    
                          </div>
                        
                           <?php if(empty($bid_winners)){ ?><button class="ask-que-btn addmoney-btn add-testmon post-botton-main btn-style-rev" type="submit"><?php  echo ($question_detials['id'])?'Update Post':'Post';?></button><?php }
						   else{
...
<?php if($this->data['question_detials']['bidding_closed_at'] < date('Y-m-d H:i:s')){?>
    <form name="commentPrize_form" action="<?php echo base_url().'superadmin/announce_prize'?>" method="post">   
    ...
    <thead class="table_heading">
	 ...
	 <tbody>  <?php 
                 ...
                 ?>
                 <h2>Total Comment Amount: <?php echo $total_comment_amt['totalAmount'];?></h2>
                 
<?php $i=1; foreach($CommentsList as $Comments){ ?>
                    <tr class="">
					<td> <?php echo $i; ?><input type="hidden" value="<?php echo $Comments['qusetion_id'];?> " name='question_id' /><input type="hidden" value="<?php echo $total_comment_amt['totalAmount'];?> " name='total_comment_amt' /></td>
                   ...
                   <?php /*?> <td><a onClick="getCommentDetails('<?php echo $Comment['id'];?>', '<?php echo $Comment['qusetion_id'];?>')" data-toggle="modal" data-target="#myModal" title="View" class="modal-ajax" rel="modal:open">View Bids</a></td><?php */?>
                    ...
                    <td>
                        ...
                        <?php $cnt = 1; while($cnt<=$Comments_count){?>
                        <option value="<?php echo $cnt;?>" <?php echo ($comment_winners[$Comments['id']]['winning_position'] == $cnt)?'selected':''?>><?php echo $cnt;?> position</option>
                        <?php $cnt++; }?>
                        </select>
                     </td>
                    <td > <a onclick='return confirm("Are you sure?")' href='<?php echo base_url('superadmin/deleteComment/'.$Comments['id'].'/'.$Comments['qusetion_id'])?>' class="fa fa-trash"></a></td>
...
$i++;}?>
<?php  if(empty($comment_winners)){?><input type="submit" class="btn btn-sm btn-success" value="Announce Prize for Comment" /><?php }?>
<?php }else{?>
 ...
 function activate_flag(cid)
{
	...
	$.ajax({
		type: "POST",
		url: '<?php echo base_url('superadmin/activate_flag'); ?>',
		data: dataString,
		success: function (data) {
			if(data == 1)
			{
...

{
	...
	$.ajax({
		...
		url: '<?php echo base_url('superadmin/getbids'); ?>',
